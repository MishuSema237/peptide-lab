                    ...
                    <div className="flex items-center justify-end space-x-2 md:space-x-4 flex-1">
                        ...
                        <div className="hidden lg:flex items-center space-x-4">
                            ...
                            {isLoading ? (
                                ...
                                <Link href="/account" className="p-2 hover:bg-gray-100 rounded-full transition-colors" aria-label="My Account">
                                    <svg className="w-5 h-5 text-dark" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                    </svg>
                                </Link>
                            ) : (
                                <Link href="/api/auth/login" className="text-dark hover:text-primary transition-colors font-medium font-sans">
                                    Login
                                </Link>
                            )}
                        ...
                        <div className="flex flex-col space-y-5 px-4">
                            ...
                            <div className="flex items-center space-x-8 pt-2">
                                <Link href="/cart" className="flex items-center space-x-2 text-dark" onClick={() => setMobileMenuOpen(false)}>
                                    ...
                                    <span className="font-medium font-sans font-bold">Cart (0)</span>
                                </Link>

                                {user ? (
                                    <Link href="/account" className="flex items-center space-x-2 text-dark" onClick={() => setMobileMenuOpen(false)}>
                                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                        </svg>
                                        <span className="font-medium font-sans font-bold">Account</span>
                                    </Link>
                                ) : (
                                    <Link href="/api/auth/login" className="flex items-center space-x-2 text-primary font-bold" onClick={() => setMobileMenuOpen(false)}>
                                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
